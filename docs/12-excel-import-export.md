...
Значения типа участника: enum-идентификаторы `KAIT_STUDENT`, `KAIT_TEACHER`,
`EXTERNAL_STUDENT`, `EXTERNAL_TEACHER`, `PARENT`, `OTHER` либо подписи
`Студент КАИТ №20`, `Сотрудник КАИТ №20`, `Студент другой организации`,
`Сотрудник другой организации`, `Родитель`, `Другое`. Multi-choice разделяется `;`, boolean
...
Экспорт участников дополнительно содержит фактический статус Participation,
роль, результат и итог баллов по мероприятию. Эти колонки не заменяют поля
Registration/Attendance и остаются пустыми для исторических мероприятий, где
Participation ещё не подтверждалось. Формульная защита применяется к ним наравне
с остальными строковыми значениями.
