...
- on the isolated Amvera beta, the same CLI may write the link to `/data`
  for the project owner rather than stdout; Amvera's project access protects
  the file, while mode 0644 allows its file-storage service to download it.
  The CLI recovers only a still-valid link for the same email and removes that
  file after activation on the next configured startup;
