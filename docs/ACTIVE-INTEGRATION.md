...
При явном завершении Event система обрабатывает активные регистрации студентов
КАИТ с подтверждённой отметкой посещения. Для нового участия роль по умолчанию
— PARTICIPANT; заранее назначенные роль и результат не заменяются. Подтверждение
и `award_score` выполняются в транзакции смены статуса Event. Ответ сообщает
число начислений и участий `NO_RULE`, чтобы отсутствие настройки не выглядело
успешным начислением. Повторное завершение идемпотентно: уже начисленные баллы
не дублируются, а `NO_RULE` можно пересчитать после исправления правил.
