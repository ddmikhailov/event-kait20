...
erDiagram
  ...
  PERSONS ||--o{ PROFILE_PUBLICATION_CONSENTS : historical_only
...
Импорт контингента создаёт PUBLIC-профиль со случайным publicSlug. Суперадминистратор
может скрыть отдельную карточку. Сведения о согласии на публикацию собираются
вне платформы; старые записи остаются в базе только как история. Согласие на
регистрацию на мероприятие продолжает обрабатываться отдельно.

Публичный рейтинг показывает только фамилию с инициалами и сумму баллов за
мероприятия в сезоне. Карточка добавляет группу, площадку из реестра и названия
мероприятий с положительным итогом начислений. Контакты, полное ФИО, роль,
результат, достижения, ручные корректировки и служебные идентификаторы не
публикуются. Групповые и отделенческие рейтинги более не доступны публично.
